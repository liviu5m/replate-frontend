...
const DonorEditDonation = () => {
  ...
  const { mutate } = useMutation({
    ...
    mutationFn: () => updateDonation(donation, Number(id), token || ""),
    onSuccess: (data) => {
      console.log(data);
  ...
  const updateDonationFunc = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    mutate();
  };
      ...
      <div className="container">
        <DashboardLayout>
          <div className="flex items-center justify-between p-5 bg-white border-b border-b-gray-400 h-20">
            <h1 className="font-semibold text-lg">Edit Donation</h1>
            <Link
              to={"/"}
          ...
          <div className="m-10 p-8 rounded-lg bg-white shadow">
            <form onSubmit={(e) => updateDonationFunc(e)}>
              <div className="mb-4">
                <label
                  htmlFor="name"
